...
from utils.utils import read_wav_np,read_wav_np_librosa
...
class MelFromDisk(Dataset):
    def __init__(self, hp, args, train):
        ...
        self.wav_list = glob.glob(os.path.join(self.path, '**', 'audio*'), recursive=True)
    ...
    def my_getitem(self, idx):
        ...
        # tmp = os.path.split(wavpath)
        # melpath = tmp[0] + '/mel-' + tmp[1].replace('.wav', '.npy')

        melpath = wavpath.replace('audio', 'mel')

        # sr, audio = read_wav_np(wavpath)
        # sr, audio = read_wav_np_librosa(wavpath, self.hp.audio.sampling_rate)

        sr = self.hp.audio.sampling_rate
        audio = np.load(wavpath)
        ...
        mel_np = np.load(melpath)
        mel_np = np.transpose(mel_np, [1, 0])
